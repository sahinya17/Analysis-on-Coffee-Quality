...
```{r expiration-date, echo = FALSE, message = FALSE, warning = FALSE}
...
The data available is for the years 2010 - 2018. Among this, 2012 has the best ratings for the harvest. 2018 had the least ratings. This is due to the favorable weather conditions in almost all the countries that produce coffee (Huong & Quan, 2012). 

```{r}
table <- best_coffee %>% group_by(harvest_year) %>% summarise(count = n())

knitr::kable(table) %>% kable_styling()
```

As it can be seen, there is only one record for 2018. Which implies that there is some missing data in the data set.  
